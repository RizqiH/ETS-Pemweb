<script lang="ts">
	...
	function filterByCategory(category: string): void {
	  ...
	  if (category === '') {
		filteredProducts = products;  // Tampilkan semua produk jika tidak ada kategori yang dipilih
	  } else {
...
<section class="products-section position-relative py-5">
	<div class="container-fluid px-4">
	  ...
	  <div class="d-flex mb-4">
		<button class="btn btn-primary me-2" on:click={() => filterByCategory('Hoodie')}>Hoodie</button>
		<button class="btn btn-primary" on:click={() => filterByCategory('Sweatshirt')}>Sweatshirt</button>
		<button class="btn btn-secondary ms-2" on:click={() => filterByCategory('')}>Tampilkan Semua</button>
	  </div>
	  ...
	  {:else}
		...
		<button class="carousel-nav-arrow carousel-nav-prev d-none d-md-flex" on:click={prevProductSlide} aria-label="Previous products">
		  <!-- Previous arrow icon here -->
		</button>
  
		<button class="carousel-nav-arrow carousel-nav-next d-none d-md-flex" on:click={nextProductSlide} aria-label="Next products">
					  ...
					  <div class="product-card h-100">
						<a href="/product/{product.id}" class="text-decoration-none text-dark">
						  <div class="product-image-container mb-3">
							<img src={product.image || 'default-image.jpg'} alt={product.name} class="product-image w-100" />
						  </div>
